...
function parseAttendanceData(rawData) {
  try {
    ...
    datas.forEach((item)=>{
      ...
      const ActualForFirstCardValue = getValue(item,'ActualForFirstCard.value','上班未打卡') // 上班打卡时间
      const ActualForLastCardValue = getValue(item,'ActualForLastCard.value','下班未打卡')  // 下班打卡时间
      const SwipingCardDate = getValue(item,'SwipingCardDate.text')
      const {workHours, Ot , tips} = getOt(DateTypeValue,ActualForFirstCardValue,ActualForLastCardValue)
      const target = {
        DateTypeValue,
...
function getOt(DateTypeValue,startTimeText, endTimeText) {
  ...
  res.Ot = isWeekend ? Math.min(Math.floor(res.workHours),8) : Math.max(0, (res.workHours - 8));
  return res
}
...
function saveAttendanceCache() {
  chrome.storage.local.set({ 'attendanceCache': attendanceCache }, function() {
    console.log('考勤数据attendanceCache已保存到本地存储');
  });
}
